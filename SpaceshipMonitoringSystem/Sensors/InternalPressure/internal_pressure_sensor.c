...
void publish_internal_pressure_value(amqp_connection_state_t *conn, const char *message) {
    ...
    if(result != 0 || amqp_get_rpc_reply(*conn).reply_type != AMQP_RESPONSE_NORMAL) {
        ...
        amqp_destroy_connection(*conn);
        *conn = connect_rabbitmq();
        publish_internal_pressure_value(conn, message);
    }
}
...
void read_and_publish_internal_pressure_value(const char *file_path) {
    ...
    while(1) {
        while(fgets(line, sizeof(line), file)) {
            ...
            publish_internal_pressure_value(&conn, line);
            sleep(3);
        }
